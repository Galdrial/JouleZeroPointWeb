...
// URL Ufficiale del Google Sheets "Pubblicato sul Web" (.tsv) di Joule Zero Point
...
// Funzione di riuso per tradurre il testo CSV nell'array di oggetti JSON delle carte
...
app.get('/api/cards', async (req, res) => {
  try {
    if (!GOOGLE_SHEETS_CSV_URL) {
      // Per i test locali prima che tu mi dia il link, usiamo il JSON generato localmente
      const mockData = require('./database_preview.json');
      return res.json(mockData);
    }

    // Modalità LIVE: scarichiamo il CSV fresco da Google Sheets
    const response = await axios.get(GOOGLE_SHEETS_CSV_URL);
    // Attenzione: Google Sheets CSV usa la virgola, non il tab (simuliamo il parsing)
    // Ma valuteremo il separatore in base a come pubblichi.
