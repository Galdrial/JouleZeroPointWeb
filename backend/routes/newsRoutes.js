 ...
 * @access  Private/Admin
 * @protocol Manages file size constraints and storage destination via Multer, with Cloudinary fallback.
 */
router.post('/admin/upload-image', adminProtect, (req, res) => {
    uploadNewsImage.single('image')(req, res, async (error) => {
        if (error) {
        ...
        // If CLOUDINARY_URL exists, we push the asset to the cloud to survive redeploys
        const cloudUrl = await uploadToCloudinary(req.file.path);
        
        if (cloudUrl) {
            // Cleanup: remove temporary local file after successful cloud transmission
